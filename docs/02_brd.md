...
| 4 | A feasibility check flagged "Debt or credit management" at 44% classifier agreement on a 50-narrative sample. A full-census follow-up (504 complaints, two independent probes — `evals/misrouting/`) sized it: 55.8% of the category doesn't belong, functioning as a general catch-all (FCRA disputes, fraud, mortgage servicing) rather than a debt-collection dump. **Resolved:** the audit was run; the finding now motivates an intake-form redesign, not a downstream classifier — see the executive brief (`docs/05_executive_brief.md`). Open sub-question: a blind precision audit to certify the 55.8% before it anchors a budget ask. | Compliance/ops analyst | Audit done; precision certification before phase 2 |
